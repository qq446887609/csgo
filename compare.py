...
df_buff = df_buff[df_buff['Buff当前在售数量']>=5] #筛选大于100在售
#只选择steam中在售数量>=100的
df_steam = df_steam[df_steam['当前在售数量']>=5]
...
df["buff库存"] = df['当前在售数量']
df["steam库存"] = df['Buff当前在售数量']
df['buff价格'] = df['Buff当前价格']
df['steam价格'] = df['饰品价格']*rate
df['倒卖赚'] = (df["steam价格"]*0.75) - df['buff价格']
df.sort_values(by = '倒卖赚').drop_duplicates()  #倒卖赚
